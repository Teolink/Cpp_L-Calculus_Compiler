...
bool Compiler::lambda_detect(const S_Storage_t::const_iterator &start, S_Storage_t::const_iterator &end) const {
	...
	if(start->type == VARIABLE) {// Case of variable
		...
		while(i != stab.cend()) {// Check for balanced parenthesization
			if(i->type == LEFT_PAR)
				stack++;
			if(i->type == RIGHT_PAR) {
				stack--;
...
Lambda* Compiler::syntax_analyze(const S_Storage_t::const_iterator &start, const S_Storage_t::const_iterator &end) {
	...
	if((i_start->type == LEFT_PAR) && ((--i_end)->type == RIGHT_PAR)) { 
		if((++i_start)->type == LAMBDA) {
			...
			if(dist >= 6) {
				if(((++i_start)->type == VARIABLE) &&
					...
					M_expr_end = i_end;
					Lambda *M_lambda = syntax_analyze(M_expr_start, M_expr_end);
					if(!M_lambda)
						return 0;
					else {
						i_start = start;
		...
		if(dist >= 4) {
			...
			M_expr_start = start;
			++M_expr_start;
			if(!lambda_detect(M_expr_start, M_expr_end)) {
				cerr << "Syntax_analysis: Could not detect first expression of term ";
				stab.print(start, end);
				cerr << endl;
				return 0;
			}
			N_expr_start = M_expr_end;
			if(!lambda_detect(N_expr_start, N_expr_end)) {
			...
			Lambda *M_lambda = syntax_analyze(M_expr_start, M_expr_end);
			Lambda *N_lambda = syntax_analyze(N_expr_start, N_expr_end);
...
void Compiler::syntax_analysis(void) {
	...
	syn_analyzed = ((lambda = syntax_analyze(stab.cbegin(), stab.cend())) != 0);
	return;
}